...
def reply(msg):
    ...
    print(ten_clock_am,now_time,two_clock)

    if ten_clock_am <= now_time <= two_clock:

        if now_time <= ten_clock_pm:
            print(now_time,next_open_time)
            if now_time < next_open_time:
                itchat.send_msg('正在计算中。。。', user)
                t1 = '34'
                t2 = '23'
                t3 = '54'
                cursor.execute("insert into data(bozhong, shoucheng, shengjiang) values (%s,%s,%s)" % (t1, t2, t3))
                conn.commit()
                itchat.send_msg('写入成功。。。', user)

            elif now_time == next_open_time:
                sleep(2)
                itchat.send_msg('=====第%d' % (int(period_num)+1) + '季收割=====', user)
            else:
                itchat.send_msg('@'+user+'播种失败，收割季节时间', user)
    ...
    #             sleep(1)  # 避免同一毫秒，代码被多次执行
        ...
        error_msg = '@'+ user + '播种有问题'
        itchat.send_msg(error_msg, user)
